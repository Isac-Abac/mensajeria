...
if ($mensajeId <= 0) {
    ...
    echo json_encode(['ok' => false, 'mensaje' => 'Mensaje invalido']);
    $conn->close();
    exit;
}
...
if (!$stmt) {
    http_response_code(500);
    echo json_encode(['ok' => false, 'mensaje' => 'Error SQL al eliminar mensaje']);
    $conn->close();
    exit;
}
...
if (!$stmt->execute()) {
    http_response_code(500);
    echo json_encode(['ok' => false, 'mensaje' => 'No se pudo eliminar el mensaje']);
    $stmt->close();
    $conn->close();
    exit;
}
...
echo json_encode(['ok' => true, 'mensaje' => 'Mensaje eliminado correctamente']);
?>
